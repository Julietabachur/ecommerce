...
const ItemDetail = ({dataProducts}) => {
  ...
  return (         
        <div key={id} className="d-flex flex-row flex-wrap align-items-center justify-content-around m-5 contenedor-detail">   
          <div id="carouselExampleControls" className="col-6 carousel slide d-flex justify-content-center align-items-center" data-bs-ride="carousel">
            <div className="carousel-inner">
            { imagenes && 
              imagenes.length > 0 && imagenes.map((image, index) =>(    
                <div className="carousel-item active ">
                  <img src={`/${image}`} key={index} className="foto-secundaria d-block img-fluid" alt=""/>
                </div>
              ))              
            }        
            </div>
          ...
          <div className="col-6 d-flex flex-column align-items-center justify-content-center border border-3 rounded-3 p-5 h-100">
